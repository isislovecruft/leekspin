...
#
# This is taken from dgoulet's tor branch bug14847_027_05, from the file
# src/test/test_hs.c.
HS_DESC_CONTENT = """\
...
def calculateSecretIDPart(permanentID, currentTime, descriptorCookie=None,
    ...
    From §1.3 torspec.git/rend-spec.txt:
      |
...
def createDescriptorCookie(length=128):
    ...
    :param int length: The length of the cookie, in bits. (default: 128)
    """
...
def generateRendServiceLine(permanentID, secretIDPart, replica=0):
    """Create the ``rendezvous-service-descriptor`` line for an HS desc.
...
def generatePermanentKey(bits=1024):
    ...
    From §1.3 of torspec.git/rend-spec.txt:
      |
...
def generateProtocolVersionsLine(version):
    ...
    From §1.8 of torspec.git/rend-spec.txt:
      |
...
def generateVersionLine(version):
    ...
    ..
    """
    # XXX which versions are which? what does this mean?
